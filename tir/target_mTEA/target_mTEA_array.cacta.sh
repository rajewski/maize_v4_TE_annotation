...
if [ ! -f ${DNATE}.tir.fa.tab ]; then
  ...
  if [ ! -s ${GENOME}.index ]; then
    ...
    target.py -q $FILENAME -t nucl -o $DNATE -i s -P $SLURM_NTASKS -S PHI -b_a 10000 -b_d 10000 -p_n 10000 -p_f 200 -p_M 0.3 $GENOME ${DNATE}_target
    echo "$(date +'%r'): Done."
  else
    echo "$(date +'%r'): TARGeT index database of genome already created."
    echo "$(date +'%r'): Running TARGeT for ${DNATE}."
    target.py -q $FILENAME -t nucl -o $DNATE -i s -P $SLURM_NTASKS -S PHI -DB -b_a 10000 -b_d 10000 -p_n 10000 -p_f 200 -p_M 0.3 $GENOME ${DNATE}_target
    echo "$(date +'%r'): Done."
  fi
